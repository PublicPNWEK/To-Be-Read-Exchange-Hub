...
(async function () {
  ...
  function formToJSON(form) {
    return Array.from(new FormData(form).entries()).reduce((o, [k, v]) => {
      o[k] = v;
      return o;
    }, {});
  }
  ...
  try {
    ...
    if (!me || !me.user || !['admin', 'staff'].includes(me.user.role)) {
  ...
  async function loadContent() {
    try {
      ...
      notify('Failed to load content: ' + e.message, 'error');
    }
  }

  function renderAffiliateLinks(links) {
    affiliateList.innerHTML = links
      .map(
        (l, i) =>
          `<li data-index="${i}"><a href="${l.url}" target="_blank" rel="noopener">${l.label || l.url}</a></li>`
      )
      .join('');
  }
  ...
  affiliateForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await fetchJSON('/api/site/affiliate-links', {
        method: 'POST',
      ...
      notify('Add failed: ' + e.message, 'error');
    }
  });
  ...
  replaceLinksBtn.addEventListener('click', async () => {
    try {
      ...
      notify('Replace failed: ' + e.message, 'error');
    }
  });
  ...
  savePolicyBtn.addEventListener('click', async () => {
    try {
      await fetchJSON('/api/site/policy', {
        method: 'PUT',
        body: JSON.stringify({ markdown: policyMarkdown.value }),
      });
      notify('Policy saved', 'success');
      await loadContent();
    } catch (e) {
      notify('Policy save failed: ' + e.message, 'error');
    }
  });
