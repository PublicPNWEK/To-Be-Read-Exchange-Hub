...
describe('Auth routes', () => {
  ...
  test('POST /api/auth/register creates user and returns token', async () => {
    pool.query.mockResolvedValueOnce({
      rows: [{ id: 1, email: 'test@example.com', display_name: 'Test User' }],
    });
    const res = await request(app)
      .post('/api/auth/register')
      .send({ email: 'test@example.com', password: 'secure123', display_name: 'Test User' });
  ...
  test('POST /api/auth/login validates credentials', async () => {
    ...
    pool.query.mockResolvedValueOnce({
      rows: [{ id: 1, email: 'user@example.com', password_hash: hash, display_name: 'User' }],
    });
    const res = await request(app)
      .post('/api/auth/login')
