...
describe('User management routes', () => {
  ...
  test('GET /api/users returns user list for admin', async () => {
    ...
    pool.query.mockResolvedValueOnce({
      rows: [{ id: 1, email: 'admin@test.com', display_name: 'Admin', is_active: true }],
    });
  ...
  test('GET /api/users/:id returns user details', async () => {
    // Mock authenticate
    pool.query.mockResolvedValueOnce({
      rows: [{ id: 2, email: 'user@test.com', display_name: 'User', is_active: true }],
    });
    ...
    const res = await request(app).get('/api/users/2').set('Authorization', `Bearer ${userToken}`);
  ...
  test('PUT /api/users/:id updates user display name', async () => {
    ...
    // Mock update
    pool.query.mockResolvedValueOnce({
      rows: [{ id: 2, email: 'user@test.com', display_name: 'New Name', is_active: true }],
    });
